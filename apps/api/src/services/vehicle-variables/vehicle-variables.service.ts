...
export class VehicleVariablesService {
  ...
  constructor(
    ...
    // this.logger.setContext(VehicleVariablesService.name); // error TS2339: Property 'setContext' does not exist on type 'AppLoggerService'.
  ...
  async fetchAll(): Promise<VehicleVariableInterface[]> {
    return await this.vehicleVariables.find({}).select('-_id -__v -createdAt -updatedAt').lean().exec();
  }
